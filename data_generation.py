...
def position_generator(position_df, batch_size=32):
    ...
    while True:
        ...
        positions = position_df.iloc[idx:idx + batch_size -1, 0].apply(fix_positions, axis=1)
        yield (positions, targets)
        idx += batch_size

def fix_positions(position_series):
    """
    Deals with weird newline thing until I can figure out why things were saved that way.
    """
    return np.fromstring(position_series.Position.replace('\n', '')[1: -1], dtype=np.float, sep=' ')
